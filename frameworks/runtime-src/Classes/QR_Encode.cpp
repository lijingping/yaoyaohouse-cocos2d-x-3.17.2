//QR andfinally.Sub. --Encode cpp file.CQR Encode implementation class
//Date andfinally 2006 / 05 / 17. Ver 1.22 Psytec Inc.
...
/////////////////////////////////////////////////////////////////////////////
// GF 2. Alpha. - 8 index number conversion table
...
/////////////////////////////////////////////////////////////////////////////
// GF 2. Alpha. - 8 integer conversion table index
...
/////////////////////////////////////////////////////////////////////////////
// Error correction generation polynomial α coefficient
...
// character number indicator bit length (by version group, {S, M, L})
...
//////////////////////////////////////////////////////////////////////////////////////////////////// //////////////////////////////////////////////////
// Construction / annihilation of QR_Encode class
...
int CQR_Encode::getmin(int a, int b)
{
	if (a < b)
		return a;
...
bool CQR_Encode::EncodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, char* lpsSource, int ncSource)
{
	...
	// Obtained by lstrlen if data length is not specified
	...
	else
	{
		...
		{
			if (bAutoExtent)
				m_nVersion = nEncodeVersion; // version (model number) automatic extension
			else
				return 0; // capacity over
		}
	}

	// Terminator code "0000" addition
	int ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[nLevel];

	int ncTerminater = getmin(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);

	if (ncTerminater > 0)
		m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 0, ncTerminater);

	// padding code "11101100, 00010001" addition
	...
	// Clear total codeword calculation area
	m_ncAllCodeWord = QR_VersonInfo[m_nVersion].ncAllCodeWord;
	memset(m_byAllCodeWord, 0, m_ncAllCodeWord);

	int nDataCwIndex = 0; // data code word processing position

	// Number of data block divisions
	...
	int nBlockNo = 0; // block being processed

	// Number of data code words per block
	...
	// Data codeword interleaved arrangement
	...
	for (i = 0; i < ncBlock2; ++i)
	{
		for (j = 0; j < ncDataCw2; ++j)
		{
			...
			{
				// 2 place block arrangement fractional arrangement
	...
	// Number of RS code words by block(*same number at present)
...
//// Application: Obtain version (model number) at encoding
//// Argument: Survey start version, encode data, encode data length
//// Returned value: Version number (capacity over = 0)
//
...
bool CQR_Encode::EncodeSourceData(char * lpsSource, int ncLength, int nVerGroup)
{
	...
	// Investigate how many characters (bytes) are continued in which mode
	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		...
		{
			// Kanji is recorded in number rather than number of characters
			++m_nBlockLength[m_ncDataBlock];
			++i;
		}
	}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////// ////////////////////////////////////
	// Join the alignment of adjacent alphanumeric mode block and numeric mode block depending on conditions

	int ncSrcBits, ncDstBits; // original bit length and single alphanumeric mode bit length when blocked

	int nBlock = 0;

	while (nBlock < m_ncDataBlock - 1)
	{
		int ncJoinFront, ncJoinBehind; // Bit length when combined with front and back 8-bit byte mode block
		int nJoinPosition = 0; // Combined with 8-bit byte mode block: -1 = join with before, 0 = not joined, 1 = join with back

		// "Array - alphanumeric" or "alphanumeric - numeric" sequence
		...
		{
			// Compare the original bit length with the bit length in case of single alphanumeric mode blocking
			...
			{
				// If there are 8-bit byte mode blocks before and after, it is checked whether coupling with them is advantageous
				if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
				{
					// 8 bit byte mode blocked before
					...
					if (ncJoinFront > ncDstBits + GetBitLength(QR_MODE_8BIT, m_nBlockLength[nBlock - 1], nVerGroup))
						ncJoinFront = 0; // Do not combine with 8 bit byte mode block
				}
				else
					ncJoinFront = 0;
				...
				{
					// with 8-bit byte mode block present
					...
					if (ncJoinBehind> ncDstBits + GetBitLength(QR_MODE_8BIT, m_nBlockLength[nBlock + 2], nVerGroup))
						ncJoinBehind = 0; // Do not combine with 8 bit byte mode block
				}
				else
					ncJoinBehind = 0;
				...
				{
					// If the 8-bit byte mode block exists both before and after, the one with the shorter data length takes precedence
				...
				if (nJoinPosition != 0)
				{
					// Combined with 8 - bit byte mode block
					if (nJoinPosition == -1)
					{
				...
				{
					// Integrates alphanumeric and numeric sequence into a single alphanumeric mode block

					if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_ALPHABET)
					{
						// join alphanumeric mode blocks following the block to be joined
						m_nBlockLength[nBlock + 1] += m_nBlockLength[nBlock + 2];
					...
					{
						// combine alphanumeric mode blocks before the joined block
						m_nBlockLength[nBlock - 1] += m_nBlockLength[nBlock];
				...
				continue; // Re-investigate the current position block
			}
		}

		++nBlock; // investigate next block
	}

	//////////////////////////////////////////////////////////////////////////////////////////////////// ////////////////////////////////////
	// Block continuous short mode blocks into 8-bit byte mode block
	...
	while (nBlock < m_ncDataBlock - 1)
	{
		...
		// If there is an 8-bit byte mode block before, subtract the redundant indicator
		if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
			ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);

		// If there is an 8-bit byte mode block behind, subtract the redundant indicator
		if (nBlock <m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
			ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);
		...
		if (ncSrcBits> ncDstBits)
		{
			if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
			{
				// Combine the 8-bit byte mode block before the joining block
				m_nBlockLength[nBlock - 1] += m_nBlockLength[nBlock];
			...
			if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
			{
				// Combine 8-bit byte mode block after the joining block
				m_nBlockLength[nBlock + 1] += m_nBlockLength[nBlock + 2];
			...
			// Re-examination before joined block
			if (nBlock >= 1)
				--nBlock;
	...
	/////////////////////////////////////////////////////////////////////////
	// Bit array
	int ncComplete = 0; // processed data counter
	unsigned short wBinCode;

	m_ncDataCodeWordBit = 0; // Bitwise processing counter
...
void CQR_Encode::GetRSCodeWord(unsigned char* lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	...
	for (i = 0; i < ncDataCodeWord; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			unsigned char nExpFirst = byIntToExp[lpbyRSWork[0]]; // Calculate the multiplier from the first term coefficient

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				// Add the first term multiplier to each term multiplier(% 255 → α ^ 255 = 1)
				unsigned char nExpElement = (unsigned char)(((int(byRSExp[ncRSCodeWord][j] + nExpFirst)) % 255));

				// Calculate the remainder by exclusive OR
...
void CQR_Encode::FormatModule()
{
	...
	if (m_nMaskingNo == -1)
	{
		// optimal masking pattern selection
		m_nMaskingNo = 0;

		SetMaskingPattern(m_nMaskingNo); // Masking
		SetFormatInfoPattern(m_nMaskingNo); // format information pattern arrangement
		...
		for (i = 1; i <= 7; ++i)
		{
			...
			SetFormatInfoPattern(i); // format information pattern arrangement
	...
	// Convert module pattern to Boolean value
...
void CQR_Encode::SetFunctionModule()
{
	...
	// Register format information description position as function module part
	for (i = 0; i < 9; ++i)
	{
...
void CQR_Encode::SetAlignmentPattern(int x, int y)
{
	...
	if (m_byModuleData[x][y] & 0x20)
		return; // Exclude because it duplicates the function module

	x -= 2; y -= 2; // Convert to upper left corner coordinates
...
void CQR_Encode::SetCodeWordPattern()
{
	...
	int nCoef_x = 1; // oriented in x axis
	int nCoef_y = 1; // oriented in the y axis direction
	...
	for (i = 0; i < m_ncAllCodeWord; ++i)
	{
		for (j = 0; j < 8; ++j)
		{
			...
			} while (m_byModuleData[x][y] & 0x20); // exclude function module
...
void CQR_Encode::SetMaskingPattern(int nPatternNo)
{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize; ++j)
		{
			if (!(m_byModuleData[j][i] & 0x20)) // Exclude functional modules
			{
				bool bMask;
...
void CQR_Encode::SetFormatInfoPattern(int nPatternNo)
{
	...
	// Arranged around the upper left position detection pattern
	for (i = 0; i <= 5; ++i)
	...
	// Arrange below the upper right position detection pattern
	for (i = 0; i <= 7; ++i)
		m_byModuleData[m_nSymbleSize - 1 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	// lower left position detection pattern right arrangement
	m_byModuleData[8][m_nSymbleSize - 8] = '\x30'; // fixed dark module
...
int CQR_Encode::CountPenalty()
{
	...
	// Adjacent module of the same color column
	for (i = 0; i < m_nSymbleSize; ++i)
	{
	...
	// Adjacent module of same color row
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 4; ++j)
		{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			...
			{
				// Before or after 4 or more bright patterns
	...
	// A pattern of 1: 1: 3: 1: 1 ratio (dark: light: dark: light: dark) in the same row
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) || (!(m_byModuleData[j - 1][i] & 0x11))) && // Light or out of symbol
				(m_byModuleData[j][i] & 0x11) && // dark - 1
				(!(m_byModuleData[j + 1][i] & 0x11)) && // Light - 1
				(m_byModuleData[j + 2][i] & 0x11) && // dark 
				(m_byModuleData[j + 3][i] & 0x11) && // dark 3
				(m_byModuleData[j + 4][i] & 0x11) && // dark 
				(!(m_byModuleData[j + 5][i] & 0x11)) && // Light - 1
				(m_byModuleData[j + 6][i] & 0x11) && // dark - 1
				((j == m_nSymbleSize - 7) || (!(m_byModuleData[j + 7][i] & 0x11))))   //Light or out of symbol
			{
				// 4 or more bright patterns before or after
	...
	// Percentage occupied by dark modules relative to the whole
	int nCount = 0;
